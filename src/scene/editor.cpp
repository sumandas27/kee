...
new_hit_obj_data::new_hit_obj_data(int key, std::size_t key_idx, float click_beat, float current_beat) :
    key(key),
    key_idx(key_idx),
    ...
    key_id(key_id)
{
    on_event = [&](ui::button::event button_event, magic_enum::containers::bitset<kee::mods> mods)
    {
        ...
        case ui::button::event::on_down_r: {
            ...
            this->editor_scene.obj_editor.ref.new_hit_object = new_hit_obj_data(this->key_id, 0, this->editor_scene.get_beat(), this->editor_scene.get_beat());
            break;
        }
...
void object_editor::on_element_mouse_move(const raylib::Vector2& mouse_pos, [[maybe_unused]] magic_enum::containers::bitset<kee::mods> mods)
{
    ...
    if (beat_drag_start.has_value() && mouse_pos != mouse_pos_start)
    {
...
bool object_editor::on_element_mouse_down(const raylib::Vector2& mouse_pos, bool is_mouse_l, magic_enum::containers::bitset<kee::mods> mods)
{
    ...
    if (!is_mouse_l)
    {
        ...
        const std::vector<int>& keys_to_render = selected_key_ids.empty() ? editor::prio_to_key : selected_key_ids;
        new_hit_object = new_hit_obj_data(keys_to_render[selected_key_idx], selected_key_idx, start_beat, start_beat);
...
void object_editor::update_element([[maybe_unused]] float dt)
{
    ...
    if (new_hit_object.has_value())
    {
        ...
        const std::vector<int>& keys_to_render = selected_key_ids.empty() ? editor::prio_to_key : selected_key_ids;
        new_hit_obj_render.emplace(make_temp_child<hit_obj_ui>(beg_beat, end_beat - beg_beat, editor_scene.get_beat(), beat_width, new_hit_object.value().key_idx, keys_to_render.size()));
...
void object_editor::attempt_move_op()
{
    ...
    for (hit_obj_node& node : hit_obj_nodes)
    {
        ...
        if (next_it != hit_objects.end() && node.new_obj.position.beat + node.new_obj.position.duration + editor::beat_lock_threshold >= next_it->first)
            is_move_valid = false;
        else if (next_it != hit_objects.begin() && std::prev(next_it)->first + std::prev(next_it)->second.duration + editor::beat_lock_threshold >= node.new_obj.position.beat)
            is_move_valid = false;
    ...
    for (hit_obj_node& node : hit_obj_nodes)
    {
        ...
        node.selected_render.render_ui.start_key_idx = is_move_valid
            ? node.selected_render.render_ui.get_key_idx()
            : node.selected_render.render_ui.start_key_idx;
    }
}
    ...
    event_history_idx(0)
{
    ...
    keys.at(KeyboardKey::KEY_Q).ref.hit_objects.emplace(0.0f, editor_hit_object(KeyboardKey::KEY_Q, 2.0f));
...
bool editor::is_key_lock_enabled() const
{
